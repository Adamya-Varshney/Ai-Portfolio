  ...
  return (
    <div className="w-full py-2 font-sans min-h-full flex flex-col gap-4">
      ...
      >
        ...
        <div className="relative flex items-center gap-4">
          <div className="relative shrink-0">
            <div className="absolute inset-0 rounded-2xl p-0.5"
              style={{ background: 'linear-gradient(135deg, #60a5fa, #93c5fd)', borderRadius: '14px' }}>
              <div className="h-full w-full rounded-2xl bg-blue-950" />
            </div>
            <div className="relative h-16 w-16 rounded-2xl overflow-hidden border-2 border-blue-400/30">
        ...
        {/* Inline summary */}
        <p className="relative mt-3 text-xs text-blue-200/80 leading-relaxed border-t border-white/10 pt-3">
      ...
      <motion.div {...fade(0.08)} className="grid grid-cols-4 gap-2">
        {stats.map(s => (
          <div key={s.value}
            ...
            style={{ background: 'linear-gradient(135deg, rgba(30,64,175,0.08), rgba(37,99,235,0.06))', borderColor: 'rgba(30,64,175,0.15)' }}
          >
      ...
      <motion.div {...fade(0.14)} className="grid grid-cols-2 gap-2">
        {highlights.map((h, i) => (
          <div key={i}
            className="rounded-xl p-3.5 flex flex-col gap-2 border hover:border-blue-300/40 transition-all duration-200 group"
            style={{ background: 'linear-gradient(135deg, rgba(30,64,175,0.05), rgba(37,99,235,0.03))', borderColor: 'rgba(30,64,175,0.12)' }}
          >
      ...
      <motion.div {...fade(0.2)} className="rounded-xl p-4 border"
        style={{ background: 'linear-gradient(145deg, rgba(30,64,175,0.05), rgba(37,99,235,0.03))', borderColor: 'rgba(30,64,175,0.12)' }}>
        <h3 className="text-[11px] font-semibold uppercase tracking-wide mb-3"
          style={{ color: '#1e40af' }}>Experience</h3>
        <div className="space-y-3">
          {timeline.map((t, i) => (
            <div key={i} className="flex items-start gap-3">
              <div className="mt-1 shrink-0 flex flex-col items-center">
                <div className="h-2 w-2 rounded-full"
                  style={{ background: t.current ? 'linear-gradient(135deg, #1e40af, #3b82f6)' : 'rgba(30,64,175,0.3)' }} />
                {i < timeline.length - 1 && <div className="w-px h-5 mt-1" style={{ background: 'rgba(30,64,175,0.15)' }} />}
              </div>
              <div className="flex-1 min-w-0">
                ...
                {t.current && (
                  <span className="ml-2 text-[10px] font-medium px-1.5 py-0.5 rounded-full"
                    style={{ background: 'rgba(30,64,175,0.1)', color: '#1e40af' }}>current</span>
                )}
              </div>
            </div>
          ))}
