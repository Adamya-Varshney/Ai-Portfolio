...
const stats = [
  { value: '3.5+ yrs', label: 'PM Experience', gradient: 'from-sky-700 to-sky-500' },
  { value: '100+ Cr', label: 'Monthly Revenue', gradient: 'from-sky-600 to-sky-400' },
  { value: '17%', label: 'Retention Lift', gradient: 'from-sky-500 to-sky-300' },
  { value: '3000+', label: 'New Sellers', gradient: 'from-sky-400 to-sky-200' },
];
  ...
  return (
    <div className="w-full py-2 font-sans min-h-full flex flex-col gap-4">
      ...
      <motion.div {...fade(0)} className="relative overflow-hidden rounded-2xl p-5"
        style={{ background: 'linear-gradient(135deg, #020e1c 0%, #072a4a 40%, #075985 100%)' }}
      >
        ...
        <div className="relative flex items-center gap-4">
          <div className="relative shrink-0">
            <div className="absolute inset-0 rounded-2xl p-0.5"
              style={{ background: 'linear-gradient(135deg, #7dd3fc, #bae6fd)', borderRadius: '14px' }}>
              <div className="h-full w-full rounded-2xl bg-sky-950" />
            </div>
            <div className="relative h-16 w-16 rounded-2xl overflow-hidden border-2 border-sky-400/30">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-sky-200 font-medium">{(profile as any).title ?? 'Product Manager'}</p>
            <div className="flex items-center gap-1 mt-1 text-[11px] text-sky-300/80">
        ...
        {/* Inline summary */}
        <p className="relative mt-3 text-xs text-sky-200/80 leading-relaxed border-t border-white/10 pt-3">
      ...
      <motion.div {...fade(0.08)} className="grid grid-cols-4 gap-2">
        {stats.map(s => (
          <div key={s.value}
            ...
            style={{ background: 'linear-gradient(135deg, rgba(2,132,199,0.08), rgba(14,165,233,0.06))', borderColor: 'rgba(2,132,199,0.15)' }}
          >
      ...
      <motion.div {...fade(0.14)} className="grid grid-cols-2 gap-2">
        {highlights.map((h, i) => (
          <div key={i}
            className="rounded-xl p-3.5 flex flex-col gap-2 border hover:border-sky-300/40 transition-all duration-200 group"
            style={{ background: 'linear-gradient(135deg, rgba(2,132,199,0.05), rgba(14,165,233,0.03))', borderColor: 'rgba(2,132,199,0.12)' }}
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-foreground leading-tight group-hover:text-sky-700 dark:group-hover:text-sky-400 transition-colors">{h.title}</span>
            </div>
      ...
      <motion.div {...fade(0.2)} className="rounded-xl p-4 border"
        style={{ background: 'linear-gradient(145deg, rgba(2,132,199,0.05), rgba(14,165,233,0.03))', borderColor: 'rgba(2,132,199,0.12)' }}>
        <h3 className="text-[11px] font-semibold uppercase tracking-wide mb-3"
          style={{ color: '#0284c7' }}>Experience</h3>
        <div className="space-y-3">
          {timeline.map((t, i) => (
            <div key={i} className="flex items-start gap-3">
              <div className="mt-1 shrink-0 flex flex-col items-center">
                <div className="h-2 w-2 rounded-full"
                  style={{ background: t.current ? 'linear-gradient(135deg, #0284c7, #38bdf8)' : 'rgba(2,132,199,0.3)' }} />
                {i < timeline.length - 1 && <div className="w-px h-5 mt-1" style={{ background: 'rgba(2,132,199,0.15)' }} />}
              </div>
              <div className="flex-1 min-w-0">
                ...
                {t.current && (
                  <span className="ml-2 text-[10px] font-medium px-1.5 py-0.5 rounded-full"
                    style={{ background: 'rgba(2,132,199,0.1)', color: '#0284c7' }}>current</span>
                )}
              </div>
            </div>
          ))}
      ...
      <motion.div {...fade(0.26)} className="flex flex-wrap gap-1.5 pb-2">
        {tags.map(tag => (
          <span key={tag}
            ...
            style={{
              background: 'linear-gradient(135deg, rgba(2,132,199,0.08), rgba(14,165,233,0.06))',
