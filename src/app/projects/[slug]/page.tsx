  ...
  return (
    <div className="min-h-screen w-full font-sans bg-white">
      ...
      <div className="w-full px-6 sm:px-12 py-10 max-w-4xl mx-auto space-y-6">
        ...
        >
          {/* Cover image */}
          <div className={`relative rounded-xl overflow-hidden mb-6 ${project.imageFit === 'contain' ? 'aspect-[16/9] bg-white' : 'h-52 sm:h-64 bg-gray-100'}`}>
            {project.images?.[0]?.src ? (
              <Image src={project.images[0].src} alt={project.title} fill className={project.imageFit === 'contain' ? 'object-contain' : 'object-cover'} />
            ) : (
